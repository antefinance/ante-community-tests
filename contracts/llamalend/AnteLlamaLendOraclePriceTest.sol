...
/// @title Checks that the LlamaLend TubbyLoan oracle never
///        returns a Tubby Cats price greater than 1 ETH.
...
///         pool never returns a Tubby Cats price greater than 1 ETH
contract AnteLlamaLendOraclePriceTest is
    AnteTest("LlamaLend oracle for TubbyLoan never returns Tubby Cats price > 1 ETH")
{
    struct Message {
        bytes32 hash;
        uint8 v;
    ...
    LendingPool public constant LLAMALEND_TUBBYLOAN_POOL = LendingPool(0x34d0A4B1265619F3cAa97608B621a17531c5626f);
    ...
    uint256 public constant FAILURE_PRICE = 1e18; // 1 ETH
    ...
    /// @notice Sets the message parameters to check for a valid signature.
    ///         As of 2022-12-07, the following API endpoint can be used to
    ///         get the latest signed message from the oracle:
    ...
    function setMessageToCheck(
        ...
        // Store the hashed message body and signature
        message = Message(
            keccak256(
