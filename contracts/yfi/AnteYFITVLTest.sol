...
contract AnteYFITVLTest is AnteTest("YFI doesn't inflate 10x over 2 months") {
    ...
    constructor () {
        protocolName = "YFI";
        testedContracts = [
                            YFI_VAULT_ADDRESS, WETH_VAULT_ADDRESS, USDC_VAULT_ADDRESS, 
    ...
    function getBalance() public view returns (uint256) {
        return YFI_CONTRACT.balanceOf(YFI_VAULT_ADDRESS)
                    + WETH_CONTRACT.balanceOf(WETH_VAULT_ADDRESS)
                    + USDC_CONTRACT.balanceOf(USDC_VAULT_ADDRESS)
                    + DAI_CONTRACT.balanceOf(DAI_VAULT_ADDRESS)
                    + WBTC_CONTRACT.balanceOf(WBTC_VAULT_ADDRESS)
