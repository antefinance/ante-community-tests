...
library VaultLifecycleTreasury {
        ...
        )
    {
        ...
        if (closeParams.currentOption == address(0)) {
            ...
            expiry = getNextExpiry(IOtoken(closeParams.currentOption).expiryTimestamp(), closeParams.period);
        }
        ...
        otokenAddress = getOrDeployOtoken(closeParams, vaultParams, underlying, asset, strikePrice, expiry, isPut);

        // get the black scholes premium of the option
        premium = GnosisAuction.getOTokenPremiumInStables(otokenAddress, optionsPremiumPricer, premiumDiscount);
    ...
    function verifyOtoken(
        ...
        require(otoken.collateralAsset() == collateralAsset, "Wrong collateralAsset");
        ...
        )
    {
        ...
        managementFeeInAsset = getManagementFee(balanceForVaultFees, vaultState.totalPending, params.managementFee);
    ...
    function createShort(
        ...
        uint256 newVaultID = (controller.getAccountVaultCounter(address(this))).add(1);
        ...
        if (oToken.isPut()) {
            ...
            mintAmount = depositAmount.mul(10**Vault.OTOKEN_DECIMALS).mul(10**18).div(
                oToken.strikePrice().mul(10**(10 + collateralDecimals))
            ); // we use 10**18 to give extra precision
        } else {
     ...
     */
    function burnOtokens(address gammaController, address currentOption) external returns (uint256) {
        ...
        actions[1] = IController.ActionArgs(
            ...
            vaultID, // vaultId
            vault.collateralAmounts[0].mul(numOTokensToBurn).div(vault.shortAmounts[0]), // amount
            0, //index
            "" //data
        );
    ...
    function getOrDeployOtoken(
        ...
        verifyOtoken(otoken, vaultParams, collateralAsset, closeParams.USDC, closeParams.delay);
    ...
    function claimAuctionOtokens(
        ...
        GnosisAuction.claimAuctionOtokens(auctionSellOrder, gnosisEasyAuction, counterpartyThetaVault);
    }
    ...
    function verifyInitializerParams(
        ...
        require(_initParams._performanceFee < 100 * Vault.FEE_MULTIPLIER, "performanceFee >= 100%");
        require(_initParams._managementFee < 100 * Vault.FEE_MULTIPLIER, "managementFee >= 100%");
        ...
        require(
            _initParams._premiumDiscount > 0 && _initParams._premiumDiscount < 100 * Vault.PREMIUM_DISCOUNT_MULTIPLIER,
            "!_premiumDiscount"
        );
        require(_initParams._auctionDuration >= _min_auction_duration, "!_auctionDuration");
     ...
     */
    function getNextExpiry(uint256 timestamp, uint256 period) internal pure returns (uint256 nextExpiry) {
        if (period == 7) {
            nextExpiry = DateTime.getNextFriday(timestamp);
            nextExpiry = nextExpiry <= timestamp ? nextExpiry + 1 weeks : nextExpiry;
        } else if (period == 14) {
            nextExpiry = DateTime.getNextFriday(timestamp);
            nextExpiry = nextExpiry <= timestamp ? nextExpiry + 2 weeks : nextExpiry;
        } else if (period == 30) {
            nextExpiry = DateTime.getMonthLastFriday(timestamp);
            nextExpiry = nextExpiry <= timestamp ? DateTime.getMonthLastFriday(nextExpiry + 1 weeks) : nextExpiry;
        } else if (period == 90) {
            nextExpiry = DateTime.getQuarterLastFriday(timestamp);
            nextExpiry = nextExpiry <= timestamp ? DateTime.getQuarterLastFriday(nextExpiry + 1 weeks) : nextExpiry;
        } else if (period == 180) {
            nextExpiry = DateTime.getBiannualLastFriday(timestamp);
            nextExpiry = nextExpiry <= timestamp ? DateTime.getBiannualLastFriday(nextExpiry + 1 weeks) : nextExpiry;
        }
