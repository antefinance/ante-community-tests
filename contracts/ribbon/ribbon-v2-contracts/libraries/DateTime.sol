...
library DateTime {
     ...
     */
    function getThisWeekFriday(uint256 timestamp) internal pure returns (uint256) {
     ...
     */
    function getLastDayOfMonth(uint256 timestamp) internal pure returns (uint256) {
        return timestampFromDate(getYear(timestamp), getMonth(timestamp) + 1, 1) - 1 days;
    }
     ...
     */
    function getMonthLastFriday(uint256 timestamp) internal pure returns (uint256) {
     ...
     */
    function getQuarterLastFriday(uint256 timestamp) internal pure returns (uint256) {
        uint256 month = getMonth(timestamp);
        uint256 quarterMonth = (month <= 3) ? 3 : (month <= 6) ? 6 : (month <= 9) ? 9 : 12;

        uint256 quarterDate = timestampFromDate(getYear(timestamp), quarterMonth, 1);
     ...
     */
    function getBiannualLastFriday(uint256 timestamp) internal pure returns (uint256) {
        uint256 month = getMonth(timestamp);
        uint256 biannualMonth = (month <= 6) ? 6 : 12;

        uint256 biannualDate = timestampFromDate(getYear(timestamp), biannualMonth, 1);
