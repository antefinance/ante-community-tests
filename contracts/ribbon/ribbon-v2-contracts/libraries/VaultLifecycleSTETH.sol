...
library VaultLifecycleSTETH {
    ...
    {
        uint256 expiry = VaultLifecycle.getNextExpiry(closeParams.currentOption);
        ...
        premium = DSMath.wmul(
            GnosisAuction.getOTokenPremium(otokenAddress, optionsPremiumPricer, premiumDiscount),
        ...
        )
    {
        ...
        newPricePerShare = ShareMath.pricePerShare(currentShareSupply, currentBalance, pendingAmount, _decimals);

        // After closing the short, if the options expire in-the-money
        // vault pricePerShare would go down because vault's asset balance decreased.
        // This ensures that the newly-minted shares do not take on the loss.
        uint256 _mintShares = ShareMath.assetToShares(pendingAmount, newPricePerShare, _decimals);

        uint256 newSupply = currentShareSupply.add(_mintShares);
        uint256 queuedAmount = newSupply > 0
            ? ShareMath.sharesToAsset(vaultState.queuedWithdrawShares, newPricePerShare, _decimals)
            : 0;

        return (currentBalance.sub(queuedAmount), queuedAmount, newPricePerShare, _mintShares);
    }
    ...
    function withdrawYieldAndBaseToken(
        ...
        if (withdrawAmount > yieldTokenBalance) {
            withdrawBaseToken(collateralToken, weth, recipient, withdrawAmount, yieldTokenBalance);
        }
    ...
    function withdrawBaseToken(
        ...
        uint256 underlyingTokensToWithdraw = IWSTETH(collateralToken).getStETHByWstETH(
    ...
    function unwrapYieldToken(
        ...
        require(amount >= minETHOut, "Amount withdrawn smaller than minETHOut from swap");
        require(minETHOut.mul(10**18).div(amount) >= 0.95 ether, "Slippage on minETHOut too high");
        ...
        // Scenario 2
        stethBalance = unwrapWstethForWithdrawal(wstEth, steth, ethBalance, stethBalance, amount, minETHOut);
        ...
        uint256 stEthAmountToSwap = DSMath.min(amount.sub(ethBalance), stethBalance);

        uint256 ethAmountOutFromSwap = swapStEthToEth(steth, crvPool, stEthAmountToSwap);
        ...
        require(totalETHOut >= minETHOut, "Output ETH amount smaller than minETHOut");
    ...
    function unwrapWstethForWithdrawal(
        ...
        if (ethstEthSum < minETHOut) {
            ...
            uint256 wstAmountToUnwrap = wstEth.getWstETHByStETH(stethNeededFromUnwrap);

            wstEth.unwrap(wstAmountToUnwrap);

            uint256 newStEthBalance = steth.balanceOf(address(this));
            require(ethBalance.add(newStEthBalance) >= minETHOut, "Unwrapping wstETH did not return sufficient stETH");
    ...
    function swapStEthToEth(
        ...
        uint256 ethAmountOutFromSwap = ICRV(crvPool).exchange(1, 0, stEthAmount, 1);
