...
contract TestVaultLifecycle {
    ...
    function getNextFriday(uint256 currentExpiry) external pure returns (uint256 nextFriday) {
        return VaultLifecycle.getNextFriday(currentExpiry);
    }

    function getNextExpiry(address currentOption) external view returns (uint256 nextExpiry) {
