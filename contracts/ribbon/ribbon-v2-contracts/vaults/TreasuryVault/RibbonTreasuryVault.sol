...
import {RibbonTreasuryVaultStorage} from "../../storage/RibbonTreasuryVaultStorage.sol";
    ...
    RibbonTreasuryVaultStorage
{
    ...
    event CollectManagementFee(uint256 managementFee, uint256 round, address indexed feeRecipient);

    event CollectPerformanceFee(uint256 performanceFee, uint256 round, address indexed feeRecipient);

    event DistributePremium(uint256 amount, uint256[] amounts, address[] recipients, uint256 round);

    event OpenShort(address indexed options, uint256 depositAmount, address indexed manager);
     ...
     */
    function initialize(VaultLifecycleTreasury.InitParams calldata _initParams, Vault.VaultParams calldata _vaultParams)
        external
        initializer
    {
        VaultLifecycleTreasury.verifyInitializerParams(_initParams, _vaultParams, MIN_AUCTION_DURATION);
    ...
    function _perRoundManagementFee(uint256 managementFee) internal view returns (uint256) {
        ...
        uint256 feeDivider = _period % 30 == 0
     ...
     */
    function setMaxDepositors(uint256 newMaxDepositors) external onlyOwner nonReentrant {
    ...
    function _addDepositor(address newDepositor) internal {
        if (!depositorsMap[newDepositor]) {
            ...
            require((depositorsArray.length + 1) <= maxDepositors, "Number of depositors exceeds limit");
        ...
        )
    {
        ...
        uint256 managementFeeInAsset;
        {
            ...
            emit CollectManagementFee(managementFeeInAsset, currentRound, recipient);
    ...
    function commitAndClose() external nonReentrant {
        ...
        VaultLifecycleTreasury.CloseParams memory closeParams = VaultLifecycleTreasury.CloseParams({
        ...
        (address otokenAddress, uint256 premium, uint256 strikePrice, uint256 delta) = VaultLifecycleTreasury
            .commitAndClose(
                strikeSelection,
    ...
    function _closeShort(address oldOption) private {
        ...
        if (oldOption != address(0)) {
            uint256 withdrawAmount = VaultLifecycleTreasury.settleShort(GAMMA_CONTROLLER);
    ...
    function rollToNextOption() external onlyKeeper nonReentrant {
        ...
        VaultLifecycleTreasury.createShort(GAMMA_CONTROLLER, MARGIN_POOL, newOption, lockedBalance);
    ...
    function burnRemainingOTokens() external onlyKeeper nonReentrant {
        uint256 unlockedAssetAmount = VaultLifecycleTreasury.burnOtokens(GAMMA_CONTROLLER, optionState.currentOption);

        vaultState.lockedAmount = uint104(uint256(vaultState.lockedAmount).sub(unlockedAssetAmount));
    }
    ...
    function _chargePerformanceFee(IERC20 token, uint256 amount) internal {
        address recipient = feeRecipient;
        uint256 transferAmount = amount.mul(performanceFee).div(100 * Vault.FEE_MULTIPLIER);
        ...
        emit CollectPerformanceFee(transferAmount, vaultState.round - 1, recipient);
    }
    ...
    function _distributePremium(IERC20 token, uint256 amount) internal {
        ...
        emit DistributePremium(amount, _amounts, _depositors, vaultState.round - 1);
    }
    ...
    function totalBalance() public view returns (uint256) {
        return uint256(vaultState.lockedAmount).add(IERC20(vaultParams.asset).balanceOf(address(this)));
    }
