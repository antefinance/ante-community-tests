...
contract RibbonDeltaVault is RibbonVault, RibbonDeltaVaultStorage {
    ...
    event OpenLong(address indexed options, uint256 purchaseAmount, uint256 premium, address indexed manager);

    event CloseLong(address indexed options, uint256 profitAmount, address indexed manager);

    event NewOptionAllocationSet(uint256 optionAllocation, uint256 newOptionAllocation);

    event InstantWithdraw(address indexed account, uint256 share, uint256 round);
    ...
    ) RibbonVault(_weth, _usdc, _gammaController, _marginPool, _gnosisEasyAuction, _uniswapRouter, _uniswapFactory) {}
    ...
    function initialize(
        ...
        require(_counterpartyThetaVault != address(0), "!_counterpartyThetaVault");
        require(
            IRibbonThetaVault(_counterpartyThetaVault).vaultParams().asset == vaultParams.asset,
            "!_counterpartyThetaVault: asset"
        );
        // 1000 = 10%. Needs to be less than 10% of the funds allocated to option.
        require(
            _optionAllocation > 0 && _optionAllocation < 10 * Vault.OPTION_ALLOCATION_MULTIPLIER,
            "!_optionAllocation"
        );
    ...
    function updatePPS(bool isWithdraw) internal {
        ...
        if (!isWithdraw || roundPricePerShare[currentRound] <= ShareMath.PLACEHOLDER_UINT) {
     ...
     */
    function setOptionAllocation(uint16 newOptionAllocation) external onlyOwner {
        // Needs to be less than 10%
        require(
            newOptionAllocation > 0 && newOptionAllocation < 10 * Vault.OPTION_ALLOCATION_MULTIPLIER,
            "Invalid allocation"
        );
    ...
    function withdrawInstantly(uint256 share) external nonReentrant {
        ...
        (uint256 sharesToWithdrawFromPending, uint256 sharesLeftForWithdrawal) = _withdrawFromNewDeposit(share);

        // Withdraw shares from pending amount
        if (sharesToWithdrawFromPending > 0) {
            vaultState.totalPending = uint128(uint256(vaultState.totalPending).sub(sharesToWithdrawFromPending));
        }
        uint256 currentRound = vaultState.round;
        ...
        if (sharesLeftForWithdrawal > 0) {
            ...
            require(sharesLeftForWithdrawal <= heldByAccount.add(heldByVault), "Insufficient balance");
        ...
        uint256 withdrawAmount = ShareMath.sharesToAsset(share, roundPricePerShare[currentRound], vaultParams.decimals);
    ...
    function commitAndClose() external nonReentrant {
        ...
        address counterpartyNextOption = counterpartyThetaVault.optionState().nextOption;
        ...
        if (oldOption != address(0)) {
            uint256 profitAmount = VaultLifecycle.settleLong(GAMMA_CONTROLLER, oldOption, vaultParams.asset);
     ...
     */
    function rollToNextOption(uint256 optionPremium) external onlyKeeper nonReentrant {
        (address newOption, uint256 lockedBalance, uint256 queuedWithdrawAmount) = _rollToNextOption(
        ...
        // place bid
        (uint256 sellAmount, uint256 buyAmount, uint64 userId) = VaultLifecycle.placeBid(bidDetails);
    ...
    function claimAuctionOtokens() external nonReentrant {
        VaultLifecycle.claimAuctionOtokens(auctionSellOrder, GNOSIS_EASY_AUCTION, address(counterpartyThetaVault));
        updatePPS(false);
    }
     ...
     */
    function _withdrawFromNewDeposit(uint256 share) private returns (uint256, uint256) {
        ...
        if (depositReceipt.round == vaultState.round && depositReceipt.amount > 0) {
            uint256 receiptShares = ShareMath.assetToShares(
