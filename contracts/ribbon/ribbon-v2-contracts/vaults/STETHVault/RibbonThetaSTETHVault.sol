...
import {RibbonThetaSTETHVaultStorage} from "../../storage/RibbonThetaSTETHVaultStorage.sol";
...
contract RibbonThetaSTETHVault is RibbonVault, RibbonThetaSTETHVaultStorage {
    ...
    constructor(
        ...
        address _crvPool
    ) RibbonVault(_weth, _usdc, _wsteth, _ldo, _gammaController, _marginPool, _gnosisEasyAuction, _crvPool) {
     ...
     */
    function setStrikeSelectionOrPricer(address newContract, bool isStrikeSelection) external onlyOwner {
     ...
     */
    function withdrawInstantly(uint256 amount, uint256 minETHOut) external nonReentrant {
        ...
        uint256 amountETHOut = VaultLifecycleSTETH.unwrapYieldToken(
            amount,
    ...
    function commitAndClose() external nonReentrant {
        ...
        (address otokenAddress, uint256 premium, uint256 strikePrice, uint256 delta) = VaultLifecycleSTETH
            .commitAndClose(
                strikeSelection,
    ...
    function rollToNextOption() external onlyKeeper nonReentrant {
        ...
        VaultLifecycleSTETH.createShort(GAMMA_CONTROLLER, MARGIN_POOL, newOption, lockedBalance);
    ...
    function burnRemainingOTokens() external onlyKeeper nonReentrant {
        ...
        vaultState.lockedAmount = uint104(
            uint256(vaultState.lockedAmount).sub(collateralToken.getStETHByWstETH(unlockedAssetAmount))
        );

        // Wrap entire `asset` balance to `collateralToken` balance
        VaultLifecycleSTETH.wrapToYieldToken(WETH, address(collateralToken), STETH);
    }
}
