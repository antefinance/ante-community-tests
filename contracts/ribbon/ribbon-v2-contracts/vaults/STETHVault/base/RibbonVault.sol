...
contract RibbonVault is ReentrancyGuardUpgradeable, OwnableUpgradeable, ERC20Upgradeable {
    ...
    function _depositFor(
        ...
        uint256 totalWithDepositedAmount = isETH ? totalBalance() : totalBalance().add(amount);
     ...
     */
    function _rollToNextOption(uint256 lastQueuedWithdrawAmount) internal returns (address, uint256) {
...

        {
            ...
            emit CollectVaultFees(performanceFeeInAsset, totalVaultFee, currentRound, vaultFeeRecipient);
            ...
            if (totalVaultFee > 0) {
                VaultLifecycleSTETH.withdrawYieldAndBaseToken(collateral, WETH, vaultFeeRecipient, totalVaultFee);
            }
        }
    ...
    function totalBalance() public view returns (uint256) {
        ...
        uint256 stethFromWsteth = collateralToken.getStETHByWstETH(collateralToken.balanceOf(address(this)));

        uint256 stEthBalance = IERC20(STETH).balanceOf(address(this));

        return wethBalance.add(vaultState.lockedAmount).add(ethBalance).add(stethFromWsteth).add(stEthBalance);
    }
