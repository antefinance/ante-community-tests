...
contract RibbonVault is ReentrancyGuardUpgradeable, OwnableUpgradeable, ERC20Upgradeable {
    ...
    function _completeWithdraw() internal returns (uint256) {
        ...
        VaultLifecycleYearn.transferAsset(WETH, vaultParams.asset, msg.sender, withdrawAmount);
    ...
    */
    function upgradeYearnVault() external onlyOwner {
        ...
        collateral.withdraw(collateral.balanceOf(address(this)), address(this), YEARN_WITHDRAWAL_SLIPPAGE);

        _upgradeYearnVault();
    }

    function _upgradeYearnVault() internal {
        address collateralAddr = IYearnRegistry(YEARN_REGISTRY).latestVault(vaultParams.asset);
