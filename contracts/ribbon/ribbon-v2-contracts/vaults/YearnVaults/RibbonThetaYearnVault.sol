...
import {RibbonThetaYearnVaultStorage} from "../../storage/RibbonThetaYearnVaultStorage.sol";
...
contract RibbonThetaYearnVault is RibbonVault, RibbonThetaYearnVaultStorage {
    ...
    ) RibbonVault(_weth, _usdc, _gammaController, _marginPool, _gnosisEasyAuction, _yearnRegistry) {
    ...
    function withdrawInstantly(uint256 amount) external nonReentrant {
        ...
        VaultLifecycleYearn.transferAsset(WETH, vaultParams.asset, msg.sender, amount);
    }
    ...
    function commitAndClose() external nonReentrant {
        ...
        (address otokenAddress, uint256 premium, uint256 strikePrice, uint256 delta) = VaultLifecycleYearn
            .commitAndClose(
                strikeSelection,
    ...
    function rollToNextOption() external onlyKeeper nonReentrant {
        (address newOption, uint256 queuedWithdrawAmount) = _rollToNextOption(uint256(lastQueuedWithdrawAmount));
        ...
        uint256 lockedBalance = collateralToken.balanceOf(address(this)).sub(
            DSMath.wdiv(
                queuedWithdrawAmount.add(queuedWithdrawAmount.mul(YEARN_WITHDRAWAL_BUFFER).div(10000)),
                collateralToken.pricePerShare().mul(VaultLifecycleYearn.decimalShift(address(collateralToken)))
            )
        );
    ...
    function burnRemainingOTokens() external onlyKeeper nonReentrant {
        ...
        VaultLifecycleYearn.wrapToYieldToken(vaultParams.asset, address(collateralToken));
    }
}
