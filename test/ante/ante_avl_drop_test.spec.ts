...
describe.only('AnteAVLDropTest', function () {
  ...
  before(async () => {
     ...
     * -----------------------------------------------
     * AnteAlwaysPassTest  1 ETH (unlocked)    500 ETH
     * AnteAlwaysPassTest  1 ETH                 1 ETH
     * AnteRevertingTest   1 ETH                 1 ETH
     */
    ...
    alwaysPassPool1 = await hre.ethers.getContractAt(
      'contracts/libraries/ante-v05-core/AntePool.sol:AntePool',
      alwaysPassPool1Addr
    );
    ...
    const factory = (await hre.ethers.getContractFactory('AnteAVLDropTest', staker)) as AnteAVLDropTest__factory;
